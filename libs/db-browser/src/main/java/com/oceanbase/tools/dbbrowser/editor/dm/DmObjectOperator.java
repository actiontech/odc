 ...
 */
package com.oceanbase.tools.dbbrowser.editor.dm;
...
/**
 * DM (DaMeng) database object operator. Uses DmSqlBuilder for identifier quoting in DROP
 * statements.
 */
public class DmObjectOperator implements DBObjectOperator {

    protected final JdbcOperations syncJdbcExecutor;

    public DmObjectOperator(JdbcOperations syncJdbcExecutor) {
        this.syncJdbcExecutor = syncJdbcExecutor;
    }

    @Override
    public void drop(DBObjectType objectType, String schemaName, String objectName) {
        String sql = GeneralSqlStatementBuilder.drop(new DmSqlBuilder(), objectType, schemaName, objectName);
