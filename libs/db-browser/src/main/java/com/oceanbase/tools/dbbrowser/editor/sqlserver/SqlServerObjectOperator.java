 ...
 */
public class SqlServerObjectOperator implements DBObjectOperator {

    protected final JdbcOperations syncJdbcExecutor;

    public SqlServerObjectOperator(JdbcOperations syncJdbcExecutor) {
        this.syncJdbcExecutor = syncJdbcExecutor;
    }

    @Override
    public void drop(DBObjectType objectType, String schemaName, String objectName) {
        String sql = GeneralSqlStatementBuilder.drop(new SqlServerSqlBuilder(), objectType, schemaName, objectName);
