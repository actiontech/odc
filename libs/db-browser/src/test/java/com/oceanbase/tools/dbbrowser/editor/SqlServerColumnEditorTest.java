...
/**
 * @description: all tests for {@link SqlServerColumnEditor}
 * @author: yizhou.xw
 * @date: 2024/12
 * @since: ODC_release_4.3.4
 */
public class SqlServerColumnEditorTest {

    private DBTableColumnEditor columnEditor;
    private final String BASE_DIR = "src/test/resources/table/operator/sqlserver/column";
    ...
    @Before
    public void setUp() {
        this.columnEditor = new SqlServerColumnEditor();
    }
    ...
    public void generateCreateObjectDDL() {
        ...
        List<DBObjectSingleTestCase<DBTableColumn>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectSingleTestCase<DBTableColumn>>>() {});
    ...
    public void generateUpdateSingleColumnDDL() {
        ...
        List<DBObjectTupleTestCase<DBTableColumn>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectTupleTestCase<DBTableColumn>>>() {});
        cases.forEach(testCase -> {
            String actual = columnEditor.generateUpdateObjectDDL(
    ...
    public void generateUpdateColumnListDDL() {
        ...
        List<DBObjectListTestCase<DBTableColumn>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectListTestCase<DBTableColumn>>>() {});
        cases.forEach(testCase -> {
            String actual = columnEditor.generateUpdateObjectListDDL(
                    testCase.getInput().getPrevious(),
                    testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
    ...
    public void generateRenameObjectDDL() {
        String casesJson = DBObjectUtilsTest.loadAsString(BASE_DIR + "/rename_column_test_cases.json");
        List<DBObjectTupleTestCase<DBTableColumn>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectTupleTestCase<DBTableColumn>>>() {});
        cases.forEach(testCase -> {
            String actual = columnEditor.generateRenameObjectDDL(
                    testCase.getInput().getPrevious(),
                    testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
