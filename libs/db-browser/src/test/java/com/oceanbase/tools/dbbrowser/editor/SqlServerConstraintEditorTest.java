...
/**
 * @description: all tests for {@link SqlServerConstraintEditor}
 * @author: yizhou.xw
 * @date: 2024/12
 * @since: ODC_release_4.3.4
 */
public class SqlServerConstraintEditorTest {

    private DBTableConstraintEditor constraintEditor;
    private final String BASE_DIR = "src/test/resources/table/operator/sqlserver/constraint";
    ...
    @Before
    public void setUp() {
        constraintEditor = new SqlServerConstraintEditor();
    }
    ...
    public void generateUpdateSingleConstraintDDL() {
        ...
        cases.forEach(testCase -> {
            String actual = constraintEditor.generateUpdateObjectDDL(
    ...
    @Test
    public void generateUpdateConstraintListDDL() {
        ...
        cases.forEach(testCase -> {
            String actual = constraintEditor.generateUpdateObjectListDDL(
                    testCase.getInput().getPrevious(),
                    testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
    ...
    public void generateRenameObjectDDL() {
        String casesJson = DBObjectUtilsTest.loadAsString(BASE_DIR + "/rename_constraint_test_cases.json");
        List<DBObjectTupleTestCase<DBTableConstraint>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectTupleTestCase<DBTableConstraint>>>() {});
        cases.forEach(testCase -> {
            String actual = constraintEditor.generateRenameObjectDDL(
                    testCase.getInput().getPrevious(),
                    testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
