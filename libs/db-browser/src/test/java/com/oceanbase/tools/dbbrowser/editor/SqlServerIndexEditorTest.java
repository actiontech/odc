...
/**
 * @description: all tests for {@link SqlServerIndexEditor}
 * @author: yizhou.xw
 * @date: 2024/12
 * @since: ODC_release_4.3.4
 */
public class SqlServerIndexEditorTest {

    private DBTableIndexEditor indexEditor;
    private final String BASE_DIR = "src/test/resources/table/operator/sqlserver/index";
    ...
    @Before
    public void setUp() {
        indexEditor = new SqlServerIndexEditor();
    }
    ...
    public void generateCreateObjectDDL() {
        ...
        List<DBObjectSingleTestCase<DBTableIndex>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectSingleTestCase<DBTableIndex>>>() {});
        cases.forEach(testCase -> {
            String actual = indexEditor.generateCreateObjectDDL(testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
    }
    ...
    @Test
    public void generateUpdateSingleIndexDDL() {
        ...
        List<DBObjectTupleTestCase<DBTableIndex>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectTupleTestCase<DBTableIndex>>>() {});
        cases.forEach(testCase -> {
            String actual = indexEditor.generateUpdateObjectDDL(
                    testCase.getInput().getPrevious(),
    ...
    public void generateUpdateIndexListDDL() {
        ...
        List<DBObjectListTestCase<DBTableIndex>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectListTestCase<DBTableIndex>>>() {});
        cases.forEach(testCase -> {
            String actual = indexEditor.generateUpdateObjectListDDL(
                    testCase.getInput().getPrevious(),
                    testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
    ...
    public void generateRenameObjectDDL() {
        String casesJson = DBObjectUtilsTest.loadAsString(BASE_DIR + "/rename_index_test_cases.json");
        List<DBObjectTupleTestCase<DBTableIndex>> cases =
                fromJson(casesJson, new TypeReference<List<DBObjectTupleTestCase<DBTableIndex>>>() {});
        cases.forEach(testCase -> {
            String actual = indexEditor.generateRenameObjectDDL(
                    testCase.getInput().getPrevious(),
                    testCase.getInput().getCurrent());
            Assert.assertEquals("Test case: " + testCase.getName(), testCase.getOutput(), actual);
        });
