 ...
 * <p>
 * Both products expose a standard PostgreSQL-compatible {@code SELECT
 * version();} call that returns a human readable build banner. The same value is the canonical
 * version string referenced elsewhere in ODC (db-browser, SQL-check rule selectors), so we return
 * it verbatim.
 */
@Extension
