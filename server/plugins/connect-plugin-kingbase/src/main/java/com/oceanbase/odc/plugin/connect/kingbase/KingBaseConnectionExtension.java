...
/**
 * KingBase connection extension.
 * <p>
 * JDBC URL: {@code jdbc:kingbase8://host:port/defaultSchema[?k=v...]} — Driver
 * {@code com.kingbase8.Driver} (not ojdbc / postgresql).
 * </p>
 */
@Extension
public class KingBaseConnectionExtension extends OBMySQLConnectionExtension {
    ...
    public String generateJdbcUrl(@NonNull JdbcUrlProperty properties) {
        ...
        Validate.notEmpty(database, "defaultSchema (database) can not be null for KingBase");

        StringBuilder jdbcUrl = new StringBuilder();
        jdbcUrl.append("jdbc:kingbase8://").append(host).append(":").append(port).append("/").append(database);
    ...
    public String getDriverClassName() {
        return OdcConstants.KINGBASE_DRIVER_CLASS_NAME;
    }
    ...
    public TestResult test(String jdbcUrl, Properties properties,
        ...
        // Ensure kingbase driver from plugin classloader is visible to DriverManager.
        try {
            Class.forName(getDriverClassName(), true, KingBaseConnectionExtension.class.getClassLoader());
    ...
    public JdbcUrlParser getConnectionInfo(@NonNull String jdbcUrl, String userName) throws SQLException {
        return new KingBaseJdbcUrlParser(jdbcUrl);
    }
