...
public class MongoJdbcUrlParser implements JdbcUrlParser {
    private static final String PREFIX = "jdbc:mongodb://";

    private final List<HostAddress> hostAddresses;
    private final Map<String, Object> parameters;
    private final String schema;

    public MongoJdbcUrlParser(@NonNull String jdbcUrl, String userName) throws SQLException {
        if (!jdbcUrl.startsWith(PREFIX)) {
            throw new SQLException("Invalid MongoDB JDBC url: " + jdbcUrl);
        }
        try {
            ...
            this.schema = uri.getPath() == null || uri.getPath().isEmpty() ? null : uri.getPath().substring(1);
            this.parameters = parseQuery(uri.getQuery());
        } catch (URISyntaxException e) {
            throw new SQLException("Failed to parse MongoDB JDBC url", e);
        }
    }
