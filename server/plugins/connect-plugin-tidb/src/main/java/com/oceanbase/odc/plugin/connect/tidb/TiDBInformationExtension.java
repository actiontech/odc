...
@Slf4j
@Extension
public class TiDBInformationExtension implements InformationExtensionPoint {
    @Override
     ...
     */
    String parseDBVersion(String version) {
        int idx = version.indexOf("-TiDB-v");
        if (idx >= 0) {
            return version.substring(idx + 7);
        }
        if (version.contains("-")) {
