...
/**
 * Hive does not surface function metadata in this release; every extension method throws
 * {@link UnsupportedOperationException} so the global exception handler maps it to HTTP 400 (the
 * front-end {@code features.functionVisible=false} flag handles the UI hide). Pins the backend
 * backstop contract (design.md §2.3 decision 1 / R-4.3).
 */
public class HiveFunctionExtensionTest {

    private final HiveFunctionExtension extension = new HiveFunctionExtension();
    ...
    public void getDetail_throwsUnsupportedOperationException() {
        Connection conn = Mockito.mock(Connection.class);
        assertThrowsUnsupported(() -> extension.getDetail(conn, "default", "f1"));
        Mockito.verifyNoInteractions(conn);
    }
    ...
    public void drop_throwsUnsupportedOperationException() {
        Connection conn = Mockito.mock(Connection.class);
        assertThrowsUnsupported(() -> extension.drop(conn, "default", "f1"));
        Mockito.verifyNoInteractions(conn);
    }

    @Test
    public void generateCreateTemplate_throwsUnsupportedOperationException() {
        assertThrowsUnsupported(() -> extension.generateCreateTemplate(new DBFunction()));
    }
