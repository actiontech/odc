...
@Extension
public class MongoDatabaseExtension implements DatabaseExtensionPoint {
    @Override
    public List<DBObjectIdentity> list(Connection connection) {
        List<DBObjectIdentity> result = new ArrayList<>();
        for (String name : MongoBridgeUtil.requireContext(connection).getClient().listDatabaseNames()) {
    ...
    public void create(Connection connection, DBDatabase database, String password) {
        throw new UnsupportedOperationException("MongoDB plugin does not support create database from ODC");
    }
}
